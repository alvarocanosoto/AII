...
def cargar():
    if index.exists_in("Index"):
        respuesta = messagebox.askyesno(title="Confirmar",message="EL INDICE YA EXISTE. \nEsta seguro que quiere recargar los datos?. \nEsta operaciÃ³n puede ser lenta")
        if respuesta:
            almacenar_datos()
    else:
        almacenar_datos()
...
def extraer_recetas():
    ...
    for i in l:
        ...
        if caracteristicas:
            caracteristicas = caracteristicas.text.replace("Caracteristicas adicionales:","")
...
def imprimir_lista_1(cursor):
    ...
    for row in cursor:
        lb.insert(END,row['titulo'])
        lb.insert(END,"    Introduccion: "+ row['introduccion'])
        lb.insert(END,"\n\n")
    lb.pack(side=LEFT,fill=BOTH)
    sc.config(command = lb.yview)
...
def buscar_titulo_o_introduccion():
    ...
    v = Toplevel()
    v.title("BÃºsqueda por Titulo o Introduccion")
        
    l1 = Label(v, text="Escriba frase del Titulo o Introduccion:")
    l1.pack(side=LEFT)
    en = Entry(v, width=75)
...
def buscar_caracteristicas_y_titulo():
    def mostrar_lista():    
        with ix.searcher() as searcher:
            entrada = '"'+str(en.get())+'"' #se ponen comillas porque hay categorias con mas de una palabra
            query = QueryParser("titulo", ix.schema).parse('caracteristicas:'+ entrada +' '+str(en1.get()))
            results = searcher.search(query,limit=10)
            imprimir_lista(results)
    
    v = Toplevel()
    v.title("BÃºsqueda por caracteristicas y Titulo")
    ...
    l1 = Label(v, text="Escriba palabras del Titulo:")
    l1.pack(side=LEFT)
...
def ventana_principal():
    ...
    buscarmenu.add_command(label="Titulo o Introduccion", command=buscar_titulo_o_introduccion)
    buscarmenu.add_command(label="Fecha", command=buscar_fecha)
    buscarmenu.add_command(label="Caracteristicas y Titulo", command=buscar_caracteristicas_y_titulo)
