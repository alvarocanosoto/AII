...
def extraer_pelicula(url):
    ...
    for link_pelicula in lista_link_peliculas:
        ...
        #si no tiene Titulo se pone el Titulo original
        if (datos.find("dt",string="Titulo")):
            titulo = datos.find("dt",string="Titulo").find_next_sibling("dd").string.strip()
        else:
            titulo = titulo_original      
        if(datos.find("dt",string="Pais")):
            pais = "".join(datos.find("dt",string="Pais").find_next_sibling("dd").stripped_strings)
        else:
            pais="Desconocido"
...
# permite buscar las Peliculas de un "Genero"
def buscar_generos():
    def mostrar_lista(event):
        ix=open_dir("Index")      
        with ix.searcher() as searcher:
            #lista de todos los Generos disponibles en el campo de Generos
            ...
            #si la entrada no estÃ¡ en la lista de Generos disponibles, da un error e informa de los Generos disponibles     
            if entrada not in lista_generos:
                messagebox.showinfo("Error", "El criterio de busqueda no es un Genero existente\nLos Generos existentes son: " + ",".join(lista_generos))
                return
    ...
    l = Label(v, text="Introduzca genero a buscar:")
    l.pack(side=LEFT)
    en = Entry(v)
    en.bind("<Return>", mostrar_lista)
    en.pack(side=LEFT)

# permite buscar una Pelicula por su Titulo y modificar su fecha de estreno
def modificar_fecha():
    def modificar():
        ...
        if(not re.match("\d{8}",en1.get())):
            ...
            return
        ix=open_dir("Index")
        lista=[]    # lista de las Peliculas a modificar, usamos el campo url (unique) para updates 
        with ix.searcher() as searcher:
            query = QueryParser("titulo", ix.schema).parse(str(en.get()))
            results = searcher.search(query, limit=None) 
            v = Toplevel()
            v.title("Listado de Peliculas a Modificar")
            v.geometry('800x150')
            sc = Scrollbar(v)
            sc.pack(side=RIGHT, fill=Y)
            lb = Listbox(v, yscrollcommand=sc.set)
            ...
            for r in results:
                ...
                lb.insert(END,'')
                lista.append(r) #cargamos la lista con los resultados de la busqueda
        # actualizamos con la nueva fecha de estreno todas las Peliculas de la lista
    ...
    l = Label(v, text="Introduzca Titulo Pelicula:")
    l.pack(side=LEFT)
    en = Entry(v)
    en.pack(side=LEFT)
