...
from tkinter import * #para poder realizar la interfaz gráfica
from tkinter import messagebox #mostar mensajes de diálogo
import sqlite3 #interactuar con la base de datos SQLite

################################DATOS#######################################################
def extraer_datos(fichero): #función para extraer datos de un csv
    try:
...
def almacenar_bd(libros): #función para almacenar datos en la BD
...
def cargar(): #función para cargar datos
...
################################LISTAR#######################################################
def listar(cursor):      
    v = Toplevel() #establece las propiedades de la ventana
    sc = Scrollbar(v)
    sc.pack(side=RIGHT, fill=Y)
    lb = Listbox(v, width=150, yscrollcommand=sc.set)
    for row in cursor: #itera sobre las filas y las añade
        s = 'TITULO: ' + row[1]
        lb.insert(END, s)
        lb.insert(END, "------------------------------------------------------------------------")
...
def listar_ordenado():
    def lista():
            conn = sqlite3.connect('books.db')
            conn.text_factory = str
            if control.get() == 1: #si radiobutton1 pulsdo
    ...
    ventana = Toplevel() #propiedades de la ventana
    control = IntVar()
...
def ventana_principal():
    raiz = Tk() #crea instancia a la venta principal de la app
    menu = Menu(raiz) #crea un menú en la ventana principal
